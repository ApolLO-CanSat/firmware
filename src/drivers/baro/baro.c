...
// adapted from
// https://github.com/raspberrypi/pico-examples/blob/4c3a3dc0196dd426fddd709616d0da984e027bab/i2c/bmp280_i2c/bmp280_i2c.c
...
typedef struct d_baro_calib_param {
  // temperature params
  uint16_t dig_t1;
  int16_t dig_t2;
  int16_t dig_t3;
...
static void d_baro_read_raw(int32_t *temp, int32_t *pressure) {
  ...
  d_i2c_write_read(ADDR, &reg, sizeof(reg), buf, sizeof(buf));
...
static int32_t d_baro_convert(int32_t temp, d_baro_calib_param *params) {
...
static int32_t d_baro_convert_temp(int32_t temp, d_baro_calib_param *params) {
  // uses the BMP280 calibration parameters to compensate the temperature value read from its registers
  int32_t t_fine = d_baro_convert(temp, params);
  return (t_fine * 5 + 128) >> 8;
}

static int32_t d_baro_convert_pressure(int32_t pressure, int32_t temp, d_baro_calib_param *params) {
...
static void d_baro_get_calib_params(d_baro_calib_param *params) {
