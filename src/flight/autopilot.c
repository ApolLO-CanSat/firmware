        ...
        if (autopilot_state.mode >= FM_GYRO) {
            if (xSemaphoreTake(autopilot_state.mutex, portMAX_DELAY) == pdTRUE) {
                if (autopilot_state.mode >= FM_ANGLE) {
                    // Update RATE targets for the 1kHz Gyro loop to follow
                    ...
                    // If we are ONLY in GYRO mode, ensure high-level rate targets are clean
                    // We do this inside the mutex so BOTH are updated at the same time for the gyro task
...
void task_gps(void *params) {
    ...
    while (1) {
        if (autopilot_state.mode >= FM_GPS_STBL) {
            if (xSemaphoreTake(autopilot_state.mutex, portMAX_DELAY) == pdTRUE) {
                if (autopilot_state.mode >= FM_GPS_STBL) {
                    // Update current horizontal positions/speeds here once driver is ready
                    // dt = 1.0f / GPS_LOOP_HZ;
                    // speed_x = (pos_x - last_pos_x) / dt;
                    // speed_y = (pos_y - last_pos_y) / dt;

                    // STBL: Target Speed = 0 (Keep position steady)
                    // float target_speed_x = pid(pos_x, target_pos_x, &pid_gps_pos_x);
                    // autopilot_state.target_pitch_angle = pid(speed_x, target_speed_x, &pid_gps_vel_x);
                    
                    // WPT: Target Speed generated by distance (Follow waypoints)
                    // float speed_limit = pid(dist_to_wpt, 0, &pid_gps_speed_limiter);
                    // autopilot_state.target_roll_angle = pid(speed_y, target_speed_y, &pid_gps_vel_y);
                }
                xSemaphoreGive(autopilot_state.mutex);
            }
        }
